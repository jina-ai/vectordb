...
def test_hnswlib_vectordb_batch(docs_to_index, call_method):
    ...
    for res in resp:
        ...
        assert res.scores[0] < 0.01  # some precision issues, should be 0.0


@pytest.mark.parametrize('call_method', ['docs', 'inputs', 'positional'])
def test_hnswlib_vectordb_single_query(docs_to_index, call_method):
    ...
    assert resp.scores[0] < 0.01  # some precision issues, should be 0.0
